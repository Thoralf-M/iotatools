...
2. **"Staking mode"** (toggle in the toolbar) — a read-only-ish view that
   ...
   commission per validator, and lets the user switch validators (or restake
   to the same validator to realize/compound accumulated rewards).
...
| `StakingTrendChart.svelte`        | ~1900 | Two modes: **My stake projection** (default) plots realized + 2 stay lines (APR linear / APY compounded) + 2 switch lines (APR / APY) + APY-based difference series + breakeven marker, with merged metrics + Switch/Restake button; **All validators** plots per-epoch returns colored by validator. Stake chips grouped by validator, sorted by total IOTA, color-graded green→red by commission, selection signaled by blue border (commission tint preserved). When ≥2 accounts own stakes, an **account list** sits above the stakes (toggles all of an account's stakes) and each account gets a distinct stable color: amount sub-chips use it as background, single-stake chips as a left accent; hovering a stake highlights its account and vice-versa. Configurable projection horizon (auto / 30 / 60 / 90 / 180 / 365 / 730). Switch-target dropdown includes the user's **current** validator(s) (marked `restake (current)`) so unstake+restake-to-same to realize/compound rewards is possible; the action button reads "Restake …" and bypasses the breakeven gate when the target is a current validator. |
...
| `transfer-transactions.ts`        | ~95   | DnD movements → transfer PTBs. `getMovements`, `prepareTransferTransactions` (one PTB per sender). Execution is driven by the global TransactionTray (`addAndRun` from `src/lib/stores/transaction-tray.ts`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `staking-transactions.ts`         | ~100  | PTB builders. `buildSwitchValidatorTransactionMulti` withdraws N stakes, **merges** the resulting coins, and stakes once → a **single** new StakedIota for one sender (caller must `tx.setSender`); `buildSwitchValidatorTransaction` is the single-stake convenience wrapper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- `userStakeRefs: UserStakeRef[]` — flat list with resolved validator. Each
  ref carries an `accountLabel` (account label or shortened address) so the
  chart's account list + per-account coloring can label stakes by owner.
...
MultiAccountView.executeSwitch
       ...
       ↓ per group: buildSwitchValidatorTransactionMulti(...) → tx.setSender(account) → addAndRun({ transaction, sender, recipients })
Each prepared tx becomes its own card in the global TransactionTray
(bottom of viewport). Cards run independently and can be re-executed
with a different mode (dry-run / send / dev-inspect / prepare).
```
...
3. Push the tx onto the global TransactionTray via
   `addAndRun({ transaction, sender, recipients, label })` from
   `src/lib/stores/transaction-tray.ts`. The tray runs each card
   independently and lets the user re-execute with a different mode.
