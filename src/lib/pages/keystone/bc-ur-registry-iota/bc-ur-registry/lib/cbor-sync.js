...
export function addWriter(format, writerFunction) {
...
export function addReader(format, readerFunction) {
    if (typeof format === 'string') {
...
export function encodeDataItem(data, format) {
    for (var i = 0; i < writerFunctions.length; i++) {
        var func = writerFunctions[i];
        var writer = func(format);
        if (writer) {
...
export function decodeToDataItem(data, format) {
    for (var i = 0; i < readerFunctions.length; i++) {
        var func = readerFunctions[i];
        var reader = func(data, format);
        if (reader) {
...
export function addSemanticEncode(tag, fn) {
...
export function addSemanticDecode(tag, fn) {
    if (typeof tag !== 'number' || tag % 1 !== 0 || tag < 0) {
        throw new Error('Tag must be a positive integer');
    }
