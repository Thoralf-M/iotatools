 ...
 * [ADAPT] Uses Address.fromPrefixedShortHex() so abbreviated 0x-prefixed
 * addresses (e.g. "0x5") validate, matching @iota/iota-sdk's isValidIotaAddress.
 * See SDK-BUGS.md: Address.fromHex() rejects anything but full 64-char hex.
 */
export function isValidIotaAddress(address: string): boolean {
    try {
        Address.fromPrefixedShortHex(address);
        return true;
    } catch {
        return false;
    }
 ...
 * [ADAPT] Uses Address.fromPrefixedShortHex() (accepts abbreviated forms and
 * left-pads) then toCanonicalString(), matching @iota/iota-sdk's
 * normalizeIotaAddress. See SDK-BUGS.md.
 */
export function normalizeIotaAddress(address: string): string {
    return Address.fromPrefixedShortHex(address).toCanonicalString(true);
}
