import { p as push, o as state, R as proxy, bz as location, s as set, u as user_effect, g as get, C as onMount, $ as onDestroy, b as sibling, h as child, i as if_block, t as template_effect, A as set_class, c as set_text, k as delegated, J as bind_value, d as append, l as pop, m as user_derived, e as each, a9 as set_style, W as set_attribute, Z as getAddressLink, U as getObjectLink, Y as comment, X as first_child, j as index, O as TransactionView, q as from_html, z as getClient, H as getSelectedNetworkConfig, B as to_array, r as delegate } from "./index-BibOl7Ks.js";
import { f as fetchRecentTransactions } from "./fetchTransactions-CvjfmVtd.js";
var root_1 = from_html(`<div class="loading">Loading initial transactions...</div>`);
var root_2 = from_html(`<div class="error"> </div>`);
var root_3 = from_html(`<div class="empty">No transactions found.</div>`);
var root_6 = from_html(`<div class="common-call svelte-knun4w"><span class="call-icon svelte-knun4w">⤷</span> <a target="_blank" rel="noopener noreferrer" class="common-call-link svelte-knun4w"> </a> <span class="call-count svelte-knun4w"> </span></div>`);
var root_7 = from_html(`<button class="tx-dot svelte-knun4w"></button>`);
...
var root_4 = from_html(`<div class="visualizer-container card svelte-knun4w"><p class="visualizer-desc svelte-knun4w">Addresses with ≥2 transactions are shown as individual rows — the move call invoked
...
var root_15 = from_html(`<a target="_blank" rel="noopener noreferrer" class="fn-link pkg-link svelte-knun4w"> </a>`);
var root_14 = from_html(`<tr><td class="col-count svelte-knun4w"> </td><td class="col-fn svelte-knun4w"><div class="fn-cell svelte-knun4w"><div class="fn-pkg-part svelte-knun4w"><!></div> <a target="_blank" rel="noopener noreferrer" class="fn-link fn-label svelte-knun4w"> </a></div></td></tr>`);
var root_12 = from_html(`<div class="move-calls-table card svelte-knun4w"><h2 class="table-title svelte-knun4w">Move Function Calls</h2> <div class="fn-table-scroll svelte-knun4w"><table class="fn-table svelte-knun4w"><thead class="svelte-knun4w"><tr class="svelte-knun4w"><th class="col-count svelte-knun4w">Count</th><th class="col-fn svelte-knun4w">Function</th></tr></thead><tbody class="svelte-knun4w"></tbody></table></div></div>`);
var root_16 = from_html(`<div class="modal-overlay svelte-knun4w" role="button" tabindex="0"><div class="modal-content svelte-knun4w" role="dialog" tabindex="-1"><!></div></div>`);
...
function TxsVisualizer($$anchor, $$props) {
  ...
  const unsubLocation = location.subscribe((v) => set(currentRoute, v, true));
  let isOnRoute = user_derived(() => get(currentRoute) === VIZ_ROUTE);
  user_effect(() => {
    if (!get(isOnRoute)) {
      if (pollTimer) {
    ...
    } else if (get(isPolling) && get(transactions).length > 0 && !pollTimer) {
      poll();
    }
  });
        ...
        if (commands && Array.isArray(commands)) {
          for (const cmd of commands) {
            ...
            if (moveCall) {
              ...
              const current = counts.get(fullCallName) || { pkg, module, func, label, fullCallName, count: 0 };
              current.count++;
              counts.set(fullCallName, current);
            }
          }
  ...
  let dotDataByDigest = user_derived(() => {
    ...
    for (const [, data] of get(otherTxsByMoveCall).withMoveCall) {
  ...
  async function poll() {
    ...
    try {
      ...
      if (get(timeDirection) === "future") {
        ...
        if (result.txs.length > 0) {
          ...
          if (fresh.length > 0) {
            set(transactions, [...get(transactions), ...fresh].slice(-get(limit)), true);
          }
        }
      } else {
        ...
        if (result.txs.length > 0) {
          ...
          const fresh = result.txs.filter((tx) => !seen.has(tx.digest));
          if (fresh.length > 0) {
            set(transactions, [...fresh, ...get(transactions)].slice(0, get(limit)), true);
          }
        }
      }
    } catch (e) {
    ...
    if (!destroyed && get(isOnRoute) && get(isPolling)) {
  ...
  async function openTransaction(tx) {
    ...
    try {
      ...
      if (get(showTransactionPopup) && get(selectedTransaction)?.digest === tx.digest) {
        set(selectedTransaction, fullTx, true);
      }
    } catch (e) {
  ...
  var node = sibling(div_1, 2);
  {
    ...
    var alternate = ($$anchor2) => {
      ...
      each(node_1, 16, () => get(multiTxAddresses), (address) => address, ($$anchor3, address) => {
        var div_14 = root_5();
        ...
        each(node_3, 17, () => get(txsBySender).get(address) || [], (tx) => tx.digest, ($$anchor4, tx) => {
      ...
      each(node_4, 17, () => get(otherTxsByMoveCall).withMoveCall, ([fullCall, data]) => fullCall, ($$anchor3, $$item) => {
        ...
        var text_12 = child(a_3);
        var span_8 = sibling(a_3, 2);
        var text_13 = child(span_8);
        var div_22 = sibling(div_20, 2);
        var node_5 = sibling(child(div_22), 2);
        each(node_5, 17, () => data().txs, (tx) => tx.digest, ($$anchor4, tx) => {
          const d = user_derived(() => get(dotDataByDigest).get(get(tx).digest));
          var button_5 = root_9();
          template_effect(
            ($0) => {
      ...
      var node_6 = sibling(node_4, 2);
      {
        var consequent_4 = ($$anchor3) => {
          ...
          var node_7 = sibling(child(div_26), 2);
          each(node_7, 17, () => get(otherTxsByMoveCall).noMoveCall, (tx) => tx.digest, ($$anchor4, tx) => {
            const d = user_derived(() => get(dotDataByDigest).get(get(tx).digest));
  ...
  var node_8 = sibling(node, 2);
  {
    var consequent_6 = ($$anchor2) => {
      ...
      each(tbody, 21, () => get(moveFunctionGroups), index, ($$anchor3, group, gi) => {
        var fragment = comment();
        var node_9 = first_child(fragment);
        each(node_9, 17, () => get(group).fns, index, ($$anchor4, entry, fi) => {
          var tr = root_14();
          set_class(tr, 1, `fn-row ${gi % 2 === 0 ? "group-even" : "group-odd"}`, "svelte-knun4w");
          var td = child(tr);
          var text_15 = child(td);
          var td_1 = sibling(td);
  ...
  delegated("click", button_3, reset);
