import { C as getDefaultExportFromCjs, at as getAugmentedNamespace, p as push, V as state, W as proxy, f as from_html, c as child, s as sibling, n as set, g as get, I as each, O as first_child, b as if_block, t as template_effect, e as set_text, k as append, a0 as user_derived, K as index, x as bind_value, j as bind_select_value, l as pop, ai as toHEX, au as update, o as getClient, Y as isValidIotaAddress, ae as Transaction, aq as messageWithIntent, av as toSerializedSignature, ap as Ed25519PublicKey, aw as to_array, B as delegate } from "/iota-utils/assets/index-vY-WO24d.js";
...
import { J as JsonToggleView } from "/iota-utils/assets/JsonToggleView-CGc85_WZ.js";
...
var root_3 = from_html(`<tr><td class="svelte-1yflp2p"> </td><td class="svelte-1yflp2p"> </td><td class="mono svelte-1yflp2p"> </td><td class="mono svelte-1yflp2p"> </td><td class="svelte-1yflp2p"> </td><td class="svelte-1yflp2p"> </td></tr>`);
var root_2 = from_html(`<tr><td colspan="2" class="svelte-1yflp2p"><table class="inner-table svelte-1yflp2p"><thead><tr><th class="svelte-1yflp2p">Index</th><th class="svelte-1yflp2p">Internal</th><th class="svelte-1yflp2p">Address</th><th class="svelte-1yflp2p">PublicKey</th><th class="svelte-1yflp2p">Balance</th><th class="svelte-1yflp2p">Owned Objects</th></tr></thead><tbody></tbody></table></td></tr>`);
var root_1 = from_html(`<tr class="clickable svelte-1yflp2p"><td class="svelte-1yflp2p"> </td><td class="svelte-1yflp2p"> </td></tr> <!>`, 1);
var root = from_html(`<main><button class="svelte-1yflp2p">connect</button> <br/> BIP 44 path: (m/44'/coinType'/accountIndex'/change'/addressIndex') <br/> <input type="number" list="coinTypes" placeholder="BIP-44 coin type" class="svelte-1yflp2p"/> <datalist id="coinTypes"><option>IOTA</option><option>Testnet</option></datalist> <input type="number" min="0" placeholder="account index" class="svelte-1yflp2p"/> <select><option>0</option><option>1</option></select> <input type="number" width="1" min="0" placeholder="address index" class="svelte-1yflp2p"/> <button class="svelte-1yflp2p">generate address</button> <br/> increase <select><option>account</option><option>address</option></select> index by: <input type="number" min="1" placeholder="number to generate" class="svelte-1yflp2p"/> <button class="svelte-1yflp2p">generate multiple addresses</button> <hr/> <button class="svelte-1yflp2p">get unknown balances</button> <button class="svelte-1yflp2p">get all balances</button> <button class="svelte-1yflp2p">get unknown objects</button> <button class="svelte-1yflp2p">get all objects</button> <hr/> <div>Sender address: <input type="string" size="70" placeholder="sender address" class="svelte-1yflp2p"/></div> <div>Recipient address: <input type="string" size="70" placeholder="recipient address" class="svelte-1yflp2p"/></div> <select><option>dry run</option><option>send</option></select> <button class="svelte-1yflp2p">send all objects</button> IOTA amount(in Nanos) to send: <input type="number" min="0" placeholder="IOTA amount to send" class="svelte-1yflp2p"/> <button class="svelte-1yflp2p">send IOTA</button> <hr/> <button class="svelte-1yflp2p">clear address list</button> <button class="svelte-1yflp2p">expand all</button> <button class="svelte-1yflp2p">collapse all</button> <!> <table class="svelte-1yflp2p"><thead><tr><th class="svelte-1yflp2p">Account</th><th class="svelte-1yflp2p">Addresses</th></tr></thead><tbody></tbody></table></main>`);
function LedgerNano($$anchor, $$props) {
  ...
  async function generateAddress() {
    try {
      ...
      let bip44Path = `m/44'/${get(coinType)}'/${get(accountIndex)}'/${get(change)}'/${get(addressIndex)}'`;
      console.log(bip44Path);
      const exists = get(accountEntries).some((entry) => entry.bip44Path === bip44Path);
      if (exists) {
        return;
      }
      ...
      get(accountEntries).push({ address, publicKey, bip44Path });
  ...
  async function generateMultipleAddresses() {
    try {
      if (get(accountOrAddress) == "account") {
        let finalIndex = get(accountIndex) + get(numberToIncrease);
        for (get(accountIndex); get(accountIndex) < finalIndex; update(accountIndex)) {
          await generateAddress();
        }
      } else {
        let finalIndex = get(addressIndex) + get(numberToIncrease);
        for (get(addressIndex); get(addressIndex) < finalIndex; update(addressIndex)) {
          await generateAddress();
        }
      }
    } catch (err) {
  ...
  function formatAsTable() {
    let grouped = [];
    for (const address of get(accountEntries)) {
      const match = address.bip44Path.match(/m\/44'\/\d+'\/(\d+)'\/(\d+)'\/(\d+)'?/);
      if (!match) {
        throw new Error("Invalid BIP44 path:" + address.bip44Path);
      }
      const accountIndex2 = parseInt(match[1]);
      const change2 = parseInt(match[2]);
      const addressIndex2 = parseInt(match[3]);
      if (!grouped[accountIndex2]) {
        grouped[accountIndex2] = [];
      }
      grouped[accountIndex2].push({
      ...
      grouped[accountIndex2].sort((a, b) => a.index - b.index);
    }
    set(tableAccounts, Object.entries(grouped).map(([key, value2]) => [parseInt(key), value2]), true);
  }
  ...
  async function getAllBalances(skipKnown = false) {
    try {
      const client = getClient();
      for (const entry of get(accountEntries)) {
        if (entry.totalBalance && skipKnown) {
          continue;
        }
        let page = await client.getBalance({ owner: entry.address });
  ...
  async function getAllObjects(skipKnown = false) {
    try {
      const client = getClient();
      for (const entry of get(accountEntries)) {
        ...
        let page = await client.getOwnedObjects({ owner: entry.address });
  ...
  async function sendAllObjects() {
    try {
      if (!isValidIotaAddress(get(senderAddress))) {
        throw new Error("invalid sender address");
      }
      if (!isValidIotaAddress(get(recipientAddress))) {
      ...
      let page = await client.getOwnedObjects({ owner: get(senderAddress), options: { showType: true } });
  ...
  async function sendIotaAmount() {
    try {
      ...
      const client = getClient();
      const tx = new Transaction();
      let balance = await client.getBalance({ owner: get(senderAddress) });
      if (BigInt(balance.totalBalance) < BigInt(get(iotaAmountToSend))) {
        throw new Error(`Not enough balance ${balance.totalBalance}/${get(iotaAmountToSend)}`);
      }
      const coins = tx.splitCoins(tx.gas, [BigInt(get(iotaAmountToSend))]);
      tx.transferObjects([coins[0]], tx.pure.address(get(recipientAddress)));
      await finishTransaction(tx, bip44Path, client);
    } catch (err) {
      set(value, err.toString(), true);
      console.error(err);
  ...
  async function finishTransaction(tx, bip44Path, client) {
    try {
      tx.setSender(get(senderAddress));
  ...
  button_1.__click = () => generateAddress();
  ...
  button_2.__click = () => generateMultipleAddresses();
  var button_3 = sibling(button_2, 4);
  button_3.__click = () => getAllBalances(true);
  var button_4 = sibling(button_3, 2);
  button_4.__click = () => getAllBalances();
  var button_5 = sibling(button_4, 2);
  button_5.__click = () => getAllObjects(true);
  var button_6 = sibling(button_5, 2);
  button_6.__click = () => getAllObjects();
  ...
  button_7.__click = () => sendAllObjects();
  var input_6 = sibling(button_7, 2);
  var button_8 = sibling(input_6, 2);
  button_8.__click = () => sendIotaAmount();
  ...
  each(tbody, 21, () => get(tableAccounts), index, ($$anchor2, $$item) => {
    ...
    var node_1 = sibling(tr, 2);
    {
      ...
      if_block(node_1, ($$render) => {
        if (isExpanded(index$1())) $$render(consequent);
      });
    }
    template_effect(
      ($0) => {
