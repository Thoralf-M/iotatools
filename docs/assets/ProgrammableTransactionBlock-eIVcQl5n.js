import { p as push, W as proxy, r as onMount, V as state, f as from_html, c as child, s as sibling, I as each, g as get, n as set, t as template_effect, S as set_class, aJ as clsx, e as set_text, h as event, k as append, K as index, d as set_attribute, x as bind_value, l as pop, v as setup_stores, o as getClient, u as store_get, ae as Transaction$1, a8 as activeAddress, af as iota_wallets, B as delegate } from "/iota-utils/assets/index-DnAm28wB.js";
import { b as bind_this } from "/iota-utils/assets/this-B6BZWJLg.js";
import { p as process } from "/iota-utils/assets/browser-BN8mR4Kt.js";
    ...
    for (; parent; { index: index2, _parent: parent } = parent) {
      if (index2 > -1)
        for (let i = index2 + dir, e = dir < 0 ? -1 : parent._tree.children.length; i != e; i += dir) {
          let child2 = parent._tree.children[i];
...
function add(elt, child2) {
  if (typeof child2 == "string") {
    ...
    for (var i = 0; i < child2.length; i++) add(elt, child2[i]);
  } else {
...
class CompositeTile extends Tile {
  ...
  sync(track) {
    ...
    for (let child2 of this.children) {
      child2.sync(track);
      length += child2.length + child2.breakAfter;
      next = prev ? prev.nextSibling : parent.firstChild;
      if (tracking && next != child2.dom)
        tracking.written = true;
      if (child2.dom.parentNode == parent) {
        while (next && next != child2.dom)
          next = rm$1(next);
      } else {
        parent.insertBefore(child2.dom, next);
      }
      prev = child2.dom;
    }
    next = prev ? prev.nextSibling : parent.firstChild;
  ...
  resolveInline(pos, side, forCoords) {
    ...
    function scan(tile, pos2) {
      for (let i = 0, off = 0; i < tile.children.length && off <= pos2; i++) {
        let child2 = tile.children[i], end = off + child2.length;
        if (end >= pos2) {
          if (child2.isComposite()) {
            scan(child2, pos2 - off);
          } else if ((!after || after.isHidden && (side > 0 || forCoords && onSameLine(after, child2))) && (end > pos2 || child2.flags & 32)) {
            after = child2;
            afterOff = pos2 - off;
          } else if (off < pos2 || child2.flags & 16 && !child2.isHidden) {
            before = child2;
            beforeOff = pos2 - off;
          }
        }
...
class TileBuilder {
  constructor(cache2, root2, blockWrappers2) {
    this.cache = cache2;
    this.root = root2;
    ...
    if (widgets.length)
      for (let i = 0, pass = 0; ; i++) {
        ...
        if (!this.reused.has(tile) && (pass == 0 ? tile.widget.compare(widget) : tile.widget.constructor == widget.constructor && widget.updateDOM(tile.dom, this.view))) {
          ...
          tile.length = length;
          tile.flags = tile.flags & -498 | flags;
          return tile;
        }
      }
  }
...
class DocView {
  ...
  updateInner(changes, composition) {
    ...
    if (this.view.viewport.from || this.view.viewport.to < this.view.state.doc.length) {
      for (let child2 of this.tile.children)
        if (child2.isWidget() && child2.widget instanceof BlockGapWidget)
  ...
  measureVisibleLineHeights(viewport) {
    ...
    let scan = (tile, pos, measureBounds) => {
      for (let i = 0; i < tile.children.length; i++) {
        if (pos > to)
          break;
        let child2 = tile.children[i], end = pos + child2.length;
        let childRect = child2.dom.getBoundingClientRect(), { height } = childRect;
        if (measureBounds && !i)
          spaceAbove += childRect.top - measureBounds.top;
        if (child2 instanceof BlockWrapperTile) {
          if (end > from)
            scan(child2, pos, childRect);
        } else if (pos >= from) {
          if (spaceAbove > 0)
  ...
  measureTextSize() {
    let lineMeasure = this.tile.blockTiles((tile) => {
      if (tile.isLine() && tile.children.length && tile.length <= 20) {
        ...
        for (let child2 of tile.children) {
          if (!child2.isText() || /[^ -~]/.test(child2.text))
            return void 0;
  ...
  lineHasWidget(pos) {
    let scan = (child2) => child2.isWidget() || child2.children.some(scan);
...
function posAtCoordsInline(view, tile, offset, x, y) {
  ...
  if (tile.isText()) {
    ...
    for (let i = 0; i < tile.children.length; i++) {
      ...
      if (child2.flags & 48)
        continue;
      let rects = (child2.dom.nodeType == 1 ? child2.dom : textRange(child2.dom, 0, child2.length)).getClientRects();
      checkRects(rects, i);
      if (!dxClosest && !dyClosest)
        break;
    }
...
function domBoundsAround(tile, from, to, offset) {
  if (tile.isComposite()) {
    ...
    for (let i = 0, pos = offset, prevEnd = offset; i < tile.children.length; i++) {
      let child2 = tile.children[i], end = pos + child2.length;
      if (pos < from && end > to)
        return domBoundsAround(child2, from, to, pos);
      if (end >= from && fromI == -1) {
        fromI = i;
        fromStart = pos;
      }
      if (pos > to && child2.dom.parentNode == tile.dom) {
        toI = i;
        toEnd = prevEnd;
        break;
      }
...
class InputState {
  ...
  ignoreDuringComposition(event2) {
    if (!/^key/.test(event2.type) || event2.synthetic)
      return false;
    if (this.composing > 0)
      return true;
...
class MouseSelection {
  ...
  select(event2) {
    let { view } = this, selection = skipAtomsForSelection(this.atoms, this.style.get(event2, this.extend, this.multiple));
...
function eventBelongsToEditor(view, event2) {
  ...
  if (event2.defaultPrevented)
    return false;
  for (let node = event2.target, tile; node != view.contentDOM; node = node.parentNode)
    if (!node || node.nodeType == 11 || (tile = Tile.get(node)) && tile.isWidget() && !tile.isHidden && tile.widget.ignoreEvent(event2))
      return false;
  return true;
}
...
function basicMouseSelection(view, event2) {
  let start = view.posAndSideAtCoords({ x: event2.clientX, y: event2.clientY }, false), type = getClickType(event2);
  ...
  return {
    ...
    get(event22, extend, multiple) {
      let cur2 = view.posAndSideAtCoords({ x: event22.clientX, y: event22.clientY }, false), removed;
...
handlers.beforeinput = (view, event2) => {
  var _a2, _b;
  if (event2.inputType == "insertText" || event2.inputType == "insertCompositionText") {
    view.inputState.insertingText = event2.data;
...
class DOMObserver {
  ...
  onSelectionChange(event2) {
    ...
    if (context && context.isWidget() && context.widget.ignoreEvent(event2)) {
      if (!wasChanged)
        this.selectionChanged = false;
      return;
    }
...
function runHandlers(map, event2, view, scope) {
  ...
  if (scopeObj) {
    ...
    !(browser.windows && event2.ctrlKey && event2.altKey) && // Alt-combinations on macOS tend to be typed characters
    !(browser.mac && event2.altKey && !(event2.ctrlKey || event2.metaKey)) && (baseName = base[event2.keyCode]) && baseName != name2) {
...
function rectangleSelectionStyle(view, event2) {
  ...
  return {
    ...
    get(event22, _extend, multiple) {
      let cur2 = getPos(view, event22);
      if (!cur2)
        return startSel;
...
const showHoverTooltipHost = /* @__PURE__ */ showTooltip.compute([showHoverTooltip], (state2) => {
  ...
  combine(values) {
    return combineConfig(values, { formatNumber: String, domEventHandlers: {} }, {
      domEventHandlers(a, b) {
        ...
        for (let event2 in b) {
          let exists = result[event2], add2 = b[event2];
          result[event2] = exists ? (view, line, event22) => exists(view, line, event22) || add2(view, line, event22) : add2;
        }
        return result;
      }
    });
...
const lineNumberGutter = /* @__PURE__ */ activeGutters.compute([lineNumberConfig], (state2) => ({
...
const activeLineGutterHighlighter = /* @__PURE__ */ gutterLineClass.compute(["selection"], (state2) => {
  let marks = [], last = -1;
  for (let range of state2.selection.ranges) {
...
class ParseContext {
  ...
  */
  static create(parser2, state2, viewport) {
    return new ParseContext(parser2, state2, [], Tree.empty, 0, viewport, [], null);
  }
  startParse() {
...
const clearSnippet = ({ state: state2, dispatch }) => {
  let active = state2.field(snippetState, false);
  if (!active)
    return false;
...
const addSnippetKeymap = /* @__PURE__ */ Prec.highest(/* @__PURE__ */ keymap.compute([snippetKeymap], (state2) => state2.facet(snippetKeymap)));
...
const deleteBracketPair = ({ state: state2, dispatch }) => {
  if (state2.readOnly)
    return false;
  ...
  let dont = null, changes = state2.changeByRange((range) => {
    if (range.empty) {
      ...
      for (let token of tokens) {
        if (token == before && nextChar(state2.doc, range.head) == closing(codePointAt(token, 0)))
          return {
...
const completionKeymapExt = /* @__PURE__ */ Prec.highest(/* @__PURE__ */ keymap.computeN([completionConfig], (state2) => state2.facet(completionConfig).defaultKeymap ? [completionKeymap] : []));
const snippets = [
...
const autoCloseTags = /* @__PURE__ */ EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
  ...
  let closeTags = state2.changeByRange((range) => {
    ...
    if (state2.doc.sliceString(head - 1, head) != text || around.name == "JSXAttributeValue" && around.to > head) ;
      ...
      let empty2 = around.parent, base22 = empty2.parent;
      if (base22 && empty2.from == head - 2 && ((name2 = elementName(state2.doc, base22.firstChild, head)) || ((_a2 = base22.firstChild) === null || _a2 === void 0 ? void 0 : _a2.name) == "JSXFragmentTag")) {
        let insert2 = `${name2}>`;
...
class HistoryState {
  ...
  addChanges(event2, time, userEvent, config2, tr) {
    ...
    userEvent == "input.type.compose")) {
      done = updateBranch(done, done.length - 1, config2.minDepth, new HistEvent(event2.changes.compose(lastEvent.changes), conc(StateEffect.mapEffects(event2.effects, lastEvent.changes), lastEvent.effects), lastEvent.mapped, lastEvent.startSelection, none));
    } else {
  ...
  pop(side, state2, onlySelection) {
    ...
    if (branch.length == 0)
      return null;
    let event2 = branch[branch.length - 1], selection = event2.selectionsAfter[0] || state2.selection;
...
const cursorMatchingBracket = ({ state: state2, dispatch }) => toMatchingBracket(state2, dispatch);
...
const selectParentSyntax = ({ state: state2, dispatch }) => {
...
const simplifySelection = ({ state: state2, dispatch }) => {
...
const splitLine = ({ state: state2, dispatch }) => {
  if (state2.readOnly)
    return false;
  let changes = state2.changeByRange((range) => {
    return {
...
const transposeChars = ({ state: state2, dispatch }) => {
  if (state2.readOnly)
    return false;
  let changes = state2.changeByRange((range) => {
...
const moveLineUp = ({ state: state2, dispatch }) => moveLine(state2, dispatch, false);
const moveLineDown = ({ state: state2, dispatch }) => moveLine(state2, dispatch, true);
...
const copyLineUp = ({ state: state2, dispatch }) => copyLine(state2, dispatch, false);
const copyLineDown = ({ state: state2, dispatch }) => copyLine(state2, dispatch, true);
...
const indentSelection = ({ state: state2, dispatch }) => {
  if (state2.readOnly)
    return false;
...
const selectWord = ({ state: state2, dispatch }) => {
...
const selectNextOccurrence = ({ state: state2, dispatch }) => {
...
const selectSelectionMatches = ({ state: state2, dispatch }) => {
  ...
  constructor(view) {
    ...
    let onkeydown = (event2) => {
      if (event2.keyCode == 27) {
        ...
        for (let i = 0; i < keys2.length; i++)
          if (keys2[i].toUpperCase().charCodeAt(0) == event2.keyCode) {
    ...
    let onclick = (event2) => {
      for (let i = 0; i < this.items.length; i++) {
        if (this.items[i].dom.contains(event2.target))
...
var root_1 = from_html(`<div><button class="svelte-i5h08g"> </button></div>`);
var root = from_html(`<main><div><div class="codeSnippet-selection" style="float: left; padding-right: 1em;"><div style="float: left; display:flexbox"><button class="svelte-i5h08g">new</button> <button class="svelte-i5h08g">rename</button> <button class="svelte-i5h08g">delete</button></div> <br/> Name: <input placeholder="string" size="15"/> <div style="border: 1px solid #dee2e6; "></div></div> <div><div class="codemirror-wrapper svelte-i5h08g"></div></div> <div><div class="codemirror-wrapper svelte-i5h08g"></div></div></div> <button class="svelte-i5h08g">dev inspect</button> <button class="svelte-i5h08g">dry run</button> <button class="svelte-i5h08g">execute</button> <!></main>`);
...
`
    ...
    return new EditorView({
      ...
      extensions: [
        ...
        EditorView.updateListener.of((v) => {
          if (v.docChanged) {
            let index2 = codeSnippets.snippets.findIndex((e) => e.name == codeSnippets.selected);
            codeSnippets.snippets[index2].code = codeEditorView.state.doc.toString();
  ...
  async function buildTransaction() {
    try {
      ...
      const scope = { Transaction: Transaction$1, client };
  ...
  function addCodeSnippet() {
    if (get(inputCodeSnippetName).length == 0) {
      set(inputCodeSnippetName, codeSnippets.snippets.length.toString(), true);
    }
    let existingIndex = codeSnippets.snippets.findIndex((e) => e.name == get(inputCodeSnippetName));
    if (existingIndex > -1) {
  ...
  function renameCodeSnippet() {
    if (get(inputCodeSnippetName).length == 0) {
      ...
      let index2 = codeSnippets.snippets.findIndex((e) => e.name == codeSnippets.selected);
      codeSnippets.snippets[index2].name = get(inputCodeSnippetName);
      codeSnippets.selected = get(inputCodeSnippetName);
      set(inputCodeSnippetName, "");
      saveCodeSnippetsToLocalstorage();
    }
  ...
  function deleteCodeSnippet() {
    if (get(inputCodeSnippetName).length == 0) {
      ...
      let index2 = codeSnippets.snippets.findIndex((e) => e.name == get(inputCodeSnippetName));
      if (index2 > -1) {
        codeSnippets.snippets.splice(index2, 1);
        saveCodeSnippetsToLocalstorage();
        if (get(inputCodeSnippetName) == codeSnippets.selected) {
  ...
  button.__click = addCodeSnippet;
  var button_1 = sibling(button, 2);
  button_1.__click = renameCodeSnippet;
  var button_2 = sibling(button_1, 2);
  button_2.__click = deleteCodeSnippet;
  var input = sibling(div_2, 4);
  var div_3 = sibling(input, 2);
  each(div_3, 21, () => codeSnippets.snippets, index, ($$anchor2, codeSnippet) => {
    var div_4 = root_1();
    var button_3 = child(div_4);
    button_3.__click = () => replaceCode(get(codeSnippet).name);
    button_3.__mouseover = () => {
    ...
    template_effect(() => {
      set_class(div_4, 1, clsx(codeSnippets.selected == get(codeSnippet).name ? "active" : ""), "svelte-i5h08g");
  ...
  bind_this(div_6, ($$value) => codeEditor = $$value, () => codeEditor);
  var div_7 = sibling(div_5, 2);
  var div_8 = child(div_7);
  bind_this(div_8, ($$value) => codeEditorPreview = $$value, () => codeEditorPreview);
  var button_4 = sibling(div, 2);
  button_4.__click = devInspect;
  var button_5 = sibling(button_4, 2);
  button_5.__click = dryRun;
  var button_6 = sibling(button_5, 2);
  button_6.__click = execute;
  var node = sibling(button_6, 2);
...
export {
  ProgrammableTransactionBlock as default
};
