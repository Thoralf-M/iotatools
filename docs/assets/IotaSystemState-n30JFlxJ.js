import { p as push, i as init, f as from_html, s as sibling, c as child, t as template_effect, g as get, m as mutable_source, a as invalidate_inner_signals, b as if_block, d as set_attribute, e as set_text, h as event, j as bind_select_value, k as append, l as pop, n as set, o as getClient, q as mutate } from "./index-DJ5RqlTl.js";
import { J as JsonToggleView } from "./JsonToggleView-y4sE79c4.js";
import { f as formatNumbersWithUnderscores } from "./iota-nano-conversion-5Mc8i2wc.js";
import "./transaction-view-B4n91x91.js";
var root_1 = from_html(`<div> </div>`);
var root = from_html(`<main><button class="svelte-801cm7">get latest IOTA system state</button> <button class="svelte-801cm7">candidate validators</button> <button class="svelte-801cm7">pending validators</button> <button class="svelte-801cm7">inactive validators</button> show full data (set before requesting): <select><option></option><option></option></select> <!> <!> <pre class="value" style="text-align: left"> </pre></main>`);
function IotaSystemState($$anchor, $$props) {
  ...
  const getInactiveValidators = async () => {
    try {
      ...
      if (parseInt(size) === 0) {
        set(value, "No inactive validators");
        return;
      }
      ...
      while (hasNextPage) {
        const inactiveValidatorsPage = await client.getDynamicFields({ parentId: inactiveValidatorsId, cursor: nextPageCursor });
        ...
        for (const inactiveValidator of inactiveValidatorsPage.data) {
          ...
          const validatorV1 = await client.getDynamicFields({
            parentId: (
              // @ts-ignore
              inactiveValidatorsPage2.data.content.fields.value.fields.inner.fields.id.id
            )
          });
          const validatorObject = await client.getObject({
            id: validatorV1.data[0].objectId,
          ...
          set(value, formatNumbersWithUnderscores(inactiveValidatorsList));
        }
