...
var root_10$2 = from_html(`<div><!></div>`);
...
async function renderSectionsToPdf(sections) {
  const [{ jsPDF }, autoTableModule] = await Promise.all([
    __vitePreload(() => import("./jspdf.es.min-tigIR8AM.js").then((n) => n.j), true ? __vite__mapDeps([0,1,2]) : void 0),
...
var root_10$1 = from_html(`<div class="preview-section-title svelte-1ubkki9"> </div>`);
              ...
              var consequent_16 = ($$anchor5) => {
                ...
                var node_29 = child(div_69);
                {
                  ...
                  if_block(node_29, ($$render) => {
                    if (get(stakeObject).rewardsByEpoch[get(filteredEpochs)[index$1()]] === "0") $$render(consequent_15);
                    else $$render(alternate_2, -1);
                  });
                }
...
function filterEpochsByTimeFrame(epochs, epochTimestamps, timeFrame, customRange, referenceDate) {
...
function getStartEpochForTimeFrame(epochTimestamps, timeFrame, customRange, referenceDate) {
...
function getTimeFrameDescription(timeFrame, customRange, referenceDate) {
...
var root_9 = from_html(`<span class="timeframe-info svelte-us09jc"> <!></span>`);
var root_11 = from_html(`<div class="info-message svelte-us09jc"> <!></div>`);
var root_13 = from_html(`<div class="loading-message svelte-us09jc">Loading can take minutes, depending on the number of transactions/epochs.</div>`);
var root_14 = from_html(`<div class="error-message svelte-us09jc"> </div>`);
var root_15 = from_html(`<div class="error-message svelte-us09jc"> </div>`);
...
function StakingRewards($$anchor, $$props) {
  ...
  async function fetchTransactions() {
    ...
    try {
      const startEpoch = getStartEpochForTimeFrame(epochTimestampsCacheJson, get(selectedTimeFrame), get(customDateRange));
      let sentDone = 0;
      let receivedDone = 0;
  ...
  legacy_pre_effect(
    () => (get(tableData), get(epochTimestamps), get(selectedTimeFrame), get(customDateRange)),
    () => {
      set(timeFrameFilteredEpochs, filterEpochsByTimeFrame(get(tableData).epochs, get(epochTimestamps), get(selectedTimeFrame), get(customDateRange)));
    }
  );
  legacy_pre_effect(
    ...
    var consequent_7 = ($$anchor2) => {
      ...
      var node_7 = sibling(text_3);
      {
        var consequent_6 = ($$anchor3) => {
          var text_4 = text();
          template_effect(() => set_text(text_4, `(${(get(timeFrameFilteredEpochs), untrack(() => get(timeFrameFilteredEpochs).length)) ?? ""} of ${(get(tableData), untrack(() => get(tableData).epochs.length)) ?? ""} epochs)`));
          append($$anchor3, text_4);
        };
        if_block(node_7, ($$render) => {
          if (get(tableData), untrack(() => get(tableData).epochs.length > 0)) $$render(consequent_6);
        });
      }
      template_effect(($0) => set_text(text_3, `${$0 ?? ""} `), [
        () => (deep_read_state(getTimeFrameDescription), get(selectedTimeFrame), get(customDateRange), untrack(() => getTimeFrameDescription(get(selectedTimeFrame), get(customDateRange))))
      ]);
      append($$anchor2, span);
    };
    if_block(node_6, ($$render) => {
      if (get(selectedTimeFrame) !== "all") $$render(consequent_7);
    });
  }
  ...
  var node_8 = sibling(div, 2);
  {
    var consequent_9 = ($$anchor2) => {
      ...
      var node_9 = sibling(text_6);
      {
        var consequent_8 = ($$anchor3) => {
          var text_7 = text();
          template_effect(() => set_text(text_7, `and ${(get(allAddresses), untrack(() => get(allAddresses).length - 3)) ?? ""} more`));
          append($$anchor3, text_7);
        };
        if_block(node_9, ($$render) => {
          if (get(allAddresses), untrack(() => get(allAddresses).length > 3)) $$render(consequent_8);
        });
      }
      template_effect(
        ($0) => set_text(text_6, `Fetching data for ${(get(allAddresses), untrack(() => get(allAddresses).length)) ?? ""} addresses: ${$0 ?? ""} `),
        [
    ...
    if_block(node_8, ($$render) => {
      if (get(allAddresses), untrack(() => get(allAddresses).length > 1)) $$render(consequent_9);
    });
  }
  var node_10 = sibling(node_8, 2);
  {
    ...
    if_block(node_10, ($$render) => {
      if (get(loadingTxs)) $$render(consequent_10);
    });
  }
  var node_11 = sibling(node_10, 2);
  {
  ...
  var node_12 = sibling(node_11, 2);
  {
    var consequent_12 = ($$anchor2) => {
      ...
      template_effect(
        ($0) => set_text(text_9, `Available Rewards went negative at ${(get(tableData), untrack(() => get(tableData).negativeAvailableEpochs.length)) ?? ""} epoch(s). This
            is most likely because stake objects were received from another address and the incoming transactions
            were not fetched${get(fetchReceivedTxs) ? " completely" : ' — try enabling "Include received" above and fetching again'}. Offending epoch${(get(tableData), untrack(() => get(tableData).negativeAvailableEpochs.length === 1 ? "" : "s")) ?? ""}: ${$0 ?? ""}${(get(tableData), untrack(() => get(tableData).negativeAvailableEpochs.length > 10 ? "…" : "")) ?? ""}`),
        [
          () => (get(tableData), untrack(() => get(tableData).negativeAvailableEpochs.slice(0, 10).join(", ")))
        ]
      );
      append($$anchor2, div_11);
    };
    if_block(node_12, ($$render) => {
      if (get(tableData), untrack(() => get(tableData).negativeAvailableEpochs.length > 0)) $$render(consequent_12);
    });
  }
  ...
  var node_14 = sibling(div_12, 2);
  {
    ...
    if_block(node_14, ($$render) => {
      if (get(stakeObjects), untrack(() => get(stakeObjects).length > 0)) $$render(consequent_13);
    });
  }
  var details = sibling(node_14, 2);
  var node_15 = sibling(child(details), 2);
  JsonToggleView(node_15, {
    get value() {
      return get(stakeObjects);
    }
  });
  var details_1 = sibling(details, 2);
  var node_16 = sibling(child(details_1), 2);
  JsonToggleView(node_16, {
    get value() {
  ...
  template_effect(() => {
    ...
    set_text(text_5, get(loadingTxs) ? get(loadingStep) ?? "Loading..." : "Fetch Data");
  });
