import { p as push, P as proxy, u as onMount, O as state, f as from_html, c as child, s as sibling, z as each, A as index, N as store_get, g as get, t as template_effect, E as bind_value, k as append, l as pop, Q as setup_stores, a7 as activeAddress, ac as iota_wallets, j as set, n as getClient, ad as Transaction$1, W as delegate, L as set_class, ae as clsx, d as set_text, e as event } from "/iota-utils/assets/index-CwZTwgfM.js";
import { b as bind_this } from "/iota-utils/assets/this-BiYEx5oF.js";
import { p as process } from "/iota-utils/assets/browser-CXrXpnWz.js";
import { T as TransactionView } from "/iota-utils/assets/TransactionView-BagsgkCQ.js";
import "/iota-utils/assets/transaction-view-MAz9PLcd.js";
import "/iota-utils/assets/b64-BgM4Sqlt.js";
import "/iota-utils/assets/attributes-CRVFLL2h.js";
import "/iota-utils/assets/bcs-gLViQVow.js";
import "/iota-utils/assets/hex-BsUxbKPD.js";
import "/iota-utils/assets/iota-nano-conversion-CfRR1_Gf.js";
...
class ContentView {
  ...
  sync(view, track) {
    if (this.flags & 2) {
      ...
      for (let child2 of this.children) {
        if (child2.flags & 7) {
          if (!child2.dom && (next = prev ? prev.nextSibling : parent.firstChild)) {
        ...
        if (track && !track.written && track.node == parent && next != child2.dom)
          track.written = true;
...
function inlineDOMAtPos(parent, pos) {
  ...
  for (let off = 0; i < children.length; i++) {
    let child2 = children[i], end = off + child2.length;
...
function coordsInChildren(view, pos, side) {
  ...
  function scan(view2, pos2) {
    for (let i = 0, off = 0; i < view2.children.length && off <= pos2; i++) {
      let child2 = view2.children[i], end = off + child2.length;
      if (end >= pos2) {
...
class DocView extends ContentView {
  ...
  coordsAt(pos, side) {
    ...
    for (let off = this.length, i = this.children.length - 1; i >= 0; i--) {
      let child2 = this.children[i], end = off - child2.breakAfter, start = end - child2.length;
      if (end < pos)
        break;
      if (start <= pos && (start < pos || child2.covers(-1)) && (end > pos || child2.covers(1)) && (!best || child2 instanceof LineView && !(best instanceof LineView && side >= 0))) {
        best = child2;
        bestPos = start;
      } else if (best && start == pos && end == pos && child2 instanceof BlockWidgetView && Math.abs(side) < 2) {
  ...
  coordsForChar(pos) {
    let { i, off } = this.childPos(pos, 1), child2 = this.children[i];
    if (!(child2 instanceof LineView))
      return null;
    while (child2.children.length) {
      let { i: i2, off: childOff } = child2.childPos(off, 1);
      for (; ; i2++) {
        if (i2 == child2.children.length)
          return null;
        if ((child2 = child2.children[i2]).length)
          break;
      }
      off = childOff;
    }
...
function domPosAtCoords(parent, x, y) {
  ...
  for (let child2 = parent.firstChild; child2; child2 = child2.nextSibling) {
    ...
    for (let i = 0; i < rects.length; i++) {
      ...
      if (dx == 0 && dy == 0)
        return child2.nodeType == 3 ? domPosInText(child2, x, y) : domPosAtCoords(child2, x, y);
...
function basicMouseSelection(view, event2) {
  ...
  return {
    ...
    get(event22, extend, multiple) {
      let cur2 = queryPos(view, event22), removed;
...
function runHandlers(map, event2, view, scope) {
  ...
  if (scopeObj) {
    ...
    !(browser.mac && event2.altKey && !event2.ctrlKey) && (baseName = base[event2.keyCode]) && baseName != name2) {
...
async function devInspect(_, buildTransaction, $activeAddress, value) {
  try {
...
async function dryRun(__1, buildTransaction, $activeAddress, value) {
  try {
    let tx = await buildTransaction();
...
async function execute(__2, buildTransaction, $iota_wallets, $activeAddress, value) {
  try {
    let tx = await buildTransaction();
...
function renameCodeSnippet(__3, inputCodeSnippetName, codeSnippets, value, saveCodeSnippetsToLocalstorage) {
...
function deleteCodeSnippet(__4, inputCodeSnippetName, codeSnippets, value, saveCodeSnippetsToLocalstorage) {
  if (get(inputCodeSnippetName).length == 0) {
...
var on_click = (__5, replaceCode, codeSnippet) => replaceCode(get(codeSnippet).name);
var on_mouseout = (__6, showPreview) => {
  set(showPreview, false);
};
var root_1 = from_html(`<div><button class="svelte-1rwiuv5"> </button></div>`);
var root = from_html(`<main><div><div class="codeSnippet-selection" style="float: left; padding-right: 1em;"><div style="float: left; display:flexbox"><button class="svelte-1rwiuv5">new</button> <button class="svelte-1rwiuv5">rename</button> <button class="svelte-1rwiuv5">delete</button></div> <br/> Name: <input placeholder="string" size="15"/> <div style="border: 1px solid #dee2e6; "></div></div> <div><div class="codemirror-wrapper svelte-1rwiuv5"></div></div> <div><div class="codemirror-wrapper svelte-1rwiuv5"></div></div></div> <button class="svelte-1rwiuv5">dev inspect</button> <button class="svelte-1rwiuv5">dry run</button> <button class="svelte-1rwiuv5">execute</button> <!></main>`);
...
`
  ...
  let activeCode = state(proxy(((_a2 = codeSnippets.snippets.find((e) => e.name == codeSnippets.selected)) == null ? void 0 : _a2.code) || ""));
  ...
  each(div_3, 21, () => codeSnippets.snippets, index, ($$anchor2, codeSnippet) => {
    ...
    button_3.__click = [on_click, replaceCode, codeSnippet];
    ...
    button_3.__mouseout = [on_mouseout, showPreview];
    var text = child(button_3);
    template_effect(() => {
      set_class(div_4, 1, clsx(codeSnippets.selected == get(codeSnippet).name ? "active" : ""), "svelte-1rwiuv5");
  ...
  button_4.__click = [devInspect, buildTransaction, $activeAddress, value];
  var button_5 = sibling(button_4, 2);
  button_5.__click = [dryRun, buildTransaction, $activeAddress, value];
  ...
  template_effect(() => {
    div_5.hidden = get(showPreview);
    div_7.hidden = !get(showPreview);
  });
