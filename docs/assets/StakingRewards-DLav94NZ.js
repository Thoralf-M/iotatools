...
async function renderSectionsToPdf(sections, onProgress) {
  const [{ jsPDF }, autoTableModule] = await Promise.all([
    __vitePreload(() => import("./jspdf.es.min-Bj9tyJ2h.js").then((n) => n.j), true ? __vite__mapDeps([0,1,2]) : void 0),
...
function StakingRewards($$anchor, $$props) {
  ...
  let skipSendersEnabled = mutable_source($sharedStakingSkipPaginationSenders().length > 0);
  ...
  async function fetchTransactions() {
    ...
    try {
      ...
      const skipSendersSnapshot = get(skipSendersEnabled) ? get(skipSendersSet) : void 0;
  ...
  legacy_pre_effect(() => (get(skipSendersEnabled), get(skipSendersTextarea)), () => {
    set(skipSendersList, get(skipSendersEnabled) ? parseAddresses(get(skipSendersTextarea)) : []);
  });
  ...
  var node_5 = sibling(node, 2);
  {
    var consequent_8 = ($$anchor2) => {
      ...
      var node_6 = sibling(label_2, 2);
      {
        ...
        if_block(node_6, ($$render) => {
          if (get(skipSendersEnabled)) $$render(consequent_7);
        });
      }
      template_effect(() => input_2.disabled = get(loadingTxs));
      bind_checked(input_2, () => get(skipSendersEnabled), ($$value) => set(skipSendersEnabled, $$value));
