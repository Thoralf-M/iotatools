const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/jspdf.es.min-fqZyBV2_.js","assets/index-DY-s66_M.js","assets/index-hJPMGCb5.css"])))=>i.map(i=>d[i]);
import { af as block, ag as EFFECT_TRANSPARENT, ah as BranchManager, ai as effect, a3 as untrack, H as getSelectedNetworkConfig, aj as IotaClient, p as push, a as prop, C as onMount, $ as onDestroy, g as get, a0 as legacy_pre_effect, a1 as deep_read_state, a2 as legacy_pre_effect_reset, v as init, b as sibling, h as child, E as bind_this, k as delegated, d as append, l as pop, y as mutable_source, s as set, D as mutate, q as from_html, r as delegate, o as state, R as proxy, u as user_effect, ak as attribute_effect, i as if_block, e as each, Y as comment, X as first_child, t as template_effect, a9 as set_style, m as user_derived, al as STYLE, am as rest_props, an as __vitePreload, A as set_class, W as set_attribute, c as set_text, a6 as bind_group, ao as bind_checked, J as bind_value, j as index, _ as text, ap as get$1, aq as sharedStakingCurrency, x as bind_select_value, a8 as copyToClipboard, w as event, ar as noop, as as NANOS_PER_IOTA, B as to_array, ab as isValidIotaAddress, at as store_set, au as sharedStakingSkipPaginationSenders, F as store_get, av as activeAddress, aw as sharedStakingSkipPaginationEnabled, G as setup_stores, aa as set_value, ax as derived_safe_equal, ay as iota_accounts } from "./index-DY-s66_M.js";
import { J as JsonToggleView } from "./JsonToggleView-BDppXqv1.js";
import { u as usePageQueryParams, a as updatePageQueryParams } from "./page-query-params-DTNeHQkz.js";
import { E as EpochPTBAnalyzer } from "./epoch-ptb-analyzer-B0Lv8GwZ.js";
import { f as fetchPoolExchangeRates, s as safeBigInt$1, g as getTokenAmount, a as getIotaAmount, b as fetchSystemState, c as getCurrentActiveValidatorsExchangeRateIds, d as getInactiveValidatorsExchangeRateIds, e as getValidatorInfo, h as fetchAllExchangeRates, i as exchangeRateCache, j as formatNanoAsIota, n as nanoToIota, k as formatNanoAsIotaFullPrecision, l as fetchEpochTimestampsForDisplay, m as action, o as setInitialExchangeRateCacheFromBinary, p as exchangeRateCacheBinary, q as fetchCurrentStakedObjects } from "./utils-Dmc8m3ll.js";
...
import "./dynamic-fields-KAd6R-dE.js";
...
async function withRetry(fn, label = "Request") {
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      const wait = backoff + Math.random() * backoff * 0.5;
      console.warn(
        `${label} failed (${err instanceof Error ? err.message : String(err)}); retrying in ${Math.round(wait)}ms (attempt ${attempt + 1}/${MAX_RETRIES$1})`
      );
      await delay(wait);
    }
  }
...
function isStakeObjectType(objectType) {
  return typeof objectType === "string" && (objectType.includes(STAKED_IOTA_MARKER) || objectType.includes(TIMELOCKED_STAKED_IOTA_MARKER));
}
...
function buildContentsJson(type, fields) {
...
function buildObjectState(pastObject) {
  if (!pastObject) return void 0;
  const type = pastObject.type ?? pastObject.content?.type ?? "";
  const ownerAddress = pastObject.owner && typeof pastObject.owner === "object" ? pastObject.owner.AddressOwner : void 0;
  return {
    asMoveObject: {
...
async function fetchPastObjects(requests) {
  ...
  await mapWithConcurrency(chunks, PAST_OBJECT_CONCURRENCY, async (chunk) => {
    const response = await withRetry(
      () => client.call("iota_tryMultiGetPastObjects", [
        chunk,
...
function buildModifiedAtMap(tx) {
  return new Map(
    (tx.effects?.modifiedAtVersions ?? []).map((entry) => [
      entry.objectId,
      entry.sequenceNumber
    ])
  );
}
function resolveChangeVersions(modifiedAt, change) {
...
async function fetchTransactionPages(address, role, startEpoch, onProgress) {
  ...
  while (hasNextPage) {
    ...
    for (const tx of page.data ?? []) {
      ...
      if (startEpoch !== void 0 && startEpoch > 0 && Number.isFinite(epoch) && epoch < startEpoch) {
...
async function fetchStakeTransactionsByRole(address, role, options = {}) {
  ...
  const candidates = rawTransactions.filter((tx) => {
    const isCandidate = (tx.objectChanges ?? []).some(
      (change) => isStakeObjectType(change.objectType)
    );
  ...
  const stakeChangeVersions = /* @__PURE__ */ new Map();
  const stakeObjectRequests = [];
  for (const tx of candidates) {
    const modifiedAt = buildModifiedAtMap(tx);
    const versions = (tx.objectChanges ?? []).filter((change) => isStakeObjectType(change.objectType)).map((change) => resolveChangeVersions(modifiedAt, change));
  ...
  const buildNode = (entry) => ({
    ...
    inputState: entry.inputVersion ? buildObjectState(
      stakeObjectContents.get(`${entry.change.objectId}@${entry.inputVersion}`)
    ) : void 0,
    outputState: entry.outputVersion ? buildObjectState(
  ...
  const payoutChangeVersions = /* @__PURE__ */ new Map();
  const payoutRequests = [];
  for (const tx of relevantTransactions) {
    const modifiedAt = buildModifiedAtMap(tx);
    const versions = (tx.objectChanges ?? []).filter(
      (change) => change.type === "created" && typeof change.objectType === "string" && (change.objectType.includes("::coin::Coin") || change.objectType.includes("::timelock::TimeLock"))
    ).map((change) => resolveChangeVersions(modifiedAt, change));
  ...
  const filteredNodes = relevantTransactions.map((tx) => {
    ...
    return {
      ...
      effects: {
        timestamp: tx.timestampMs ? new Date(Number(tx.timestampMs)).toISOString() : void 0,
...
async function fetchStakeTransactions(address, options) {
  return fetchStakeTransactionsByRole(address, "FromAddress", options);
}
async function fetchReceivedStakeTransactions(address, options) {
...
const pricesCache = {
  ...
  "06-07-2026": { "usd": 0.03819206785583141, "eur": 0.03339678639205678 },
  "07-07-2026": { "usd": 0.03786349108893336, "eur": 0.03309094949113766 },
  "08-07-2026": { "usd": 0.03635197566486314, "eur": 0.031880573602157986 },
  "09-07-2026": { "usd": 0.03639384499288946, "eur": 0.031867688066503766 },
  "10-07-2026": { "usd": 0.03681682397722031, "eur": 0.0322031973141629 },
  "11-07-2026": { "usd": 0.03907221126219118, "eur": 0.03421705921854005 },
  "12-07-2026": { "usd": 0.03946839865785066, "eur": 0.03457439616107449 },
  "13-07-2026": { "usd": 0.038451218668143265, "eur": 0.03372760180841787 },
  "14-07-2026": { "usd": 0.0366661170320574, "eur": 0.03221162380606681 },
  "15-07-2026": { "usd": 0.036793878883924144, "eur": 0.03220549821770439 },
  "16-07-2026": { "usd": 0.03720811408960658, "eur": 0.032437289701037215 }
};
const FONT_SIZE = 8;
...
async function renderSectionsToPdf(sections, onProgress) {
  const [{ jsPDF }, autoTableModule] = await Promise.all([
    __vitePreload(() => import("./jspdf.es.min-fqZyBV2_.js").then((n) => n.j), true ? __vite__mapDeps([0,1,2]) : void 0),
    ...
    try {
      ...
      if (res.status === 429) {
        if (attempt < MAX_RETRIES) {
          const delay2 = RETRY_DELAY_MS * Math.pow(2, attempt);
          console.warn(
            `Rate limited for date ${dateStr}, retrying in ${delay2}ms (attempt ${attempt + 1}/${MAX_RETRIES + 1})`
          );
          await applyRateLimit(delay2);
          continue;
        } else {
          console.warn(
      ...
      if (attempt < MAX_RETRIES) {
        const delay2 = RETRY_DELAY_MS * Math.pow(2, attempt);
        console.warn(
          `Network error for date ${dateStr}: ${error instanceof Error ? error.message : String(error)}, retrying in ${delay2}ms (attempt ${attempt + 1}/${MAX_RETRIES + 1})`
        );
        await applyRateLimit(delay2);
        continue;
      } else {
...
var root_7 = from_html(
  `<label class="toggle-row svelte-us09jc" style="margin-top: 0.5rem;"><div class="toggle-switch svelte-us09jc"><input type="checkbox" class="svelte-us09jc"/> <span class="slider svelte-us09jc"></span></div> <div style="display: flex; flex-direction: column; line-height: 1.2;"><div style="display: flex; align-items: center; gap: 0.25rem;"><span class="toggle-label">Skip object changes by sender</span> <div class="tooltip-container svelte-us09jc"><span class="info-icon svelte-us09jc">ⓘ</span> <div class="tooltip svelte-us09jc">Transactions from these addresses are skipped when looking for
                                    ...
                                    transactions and would otherwise dominate fetch time.</div></div></div> <span style="font-size: 0.75rem; opacity: 0.7;">Speeds up received-tx fetch for known-noisy senders</span></div></label> <!>`,
  1
);
...
var root_22 = from_html(`<div class="info-message svelte-us09jc"> <!></div>`);
var root_24 = from_html(`<div class="loading-message svelte-us09jc">Loading can take minutes, depending on the number of transactions/epochs.</div>`);
...
var root_27 = from_html(`<div class="info-message svelte-us09jc"> </div>`);
...
function StakingRewards($$anchor, $$props) {
  ...
  var node_24 = sibling(child(details_1), 2);
  JsonToggleView(node_24, {
    get value() {
      return get(stakeObjects);
    }
  });
  var details_2 = sibling(details_1, 2);
  var node_25 = sibling(child(details_2), 2);
